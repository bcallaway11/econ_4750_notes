...
\item
  ...
  \item
    Access:
    \href{http://bcallaway11.github.io/Courses/ECON_4750_Fall_2025/}{Course
    Website}
  \item
    Description:
    \href{http://bcallaway11.github.io/Courses/ECON_4750_Fall_2025/}{Course
...
I am not going to provide a proof of Chebyshev's Inequality. However, it
is not that complicated. It follows from another result called Markov's
Inequality, where the idea is basically that, if you know the mean of a
random variable, it limits the probability that a random variable can
take an extreme value. Chebyshev's Inequality extends this idea to the
case where you know the mean and standard deviation of a random
variable.
...
\begin{equation}\phantomsection\label{eq-ovb-y}{
  Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 Q + U 
}\end{equation} where \(\E[U|X_1,X_2,Q]=0\).
...
notice if we plug in the expression for \(Y\) from
Equation~\ref{eq-ovb-y},
...
\begin{equation}\phantomsection\label{eq-foc-bet0}{
  -\frac{2}{n}\sum_{i=1}^n (Y_i - \hat{\beta}_0 - \hat{\beta}_1 X_i) = 0 
}\end{equation} where the \(2\) comes from taking the deriviative of the
squared part and the negative sign at the beginning comes from taking
the derivative of \(-b_0\) on the inside. I also replaced \(b_0\) and
\(b_1\) here since \(\hat{\beta}_0\) and \(\hat{\beta}_1\) are the
values that will solve this equation.

Next, let's take the derivative with respect to \(b_1\):
\begin{equation}\phantomsection\label{eq-foc-bet1}{
  -\frac{2}{n} \sum_{i=1}^n (Y_i - \hat{\beta}_0 - \hat{\beta}_1 X_i) X_i = 0 
}\end{equation} where the 2 comes from taking the derivative of the
squared part, and the negative sign at the beginning and \(X_i\) at the
end come from taking the derivative of the inside with respect to
\(b_1\).

All we have to do now is to solve Equation~\ref{eq-foc-bet0} and
Equation~\ref{eq-foc-bet1} for \(\hat{\beta}_0\) and \(\hat{\beta}_1\).
Before we do it, let me just mention that we are about to do some fairly
...
To start with, recall that \begin{equation}\phantomsection\label{eq-b1}{
...
This implies that \begin{equation}\phantomsection\label{eq-hr1}{
  \frac{1}{n}\sum_{i=1}^n \Big( (X_i - \bar{X})(Y_i - \bar{Y})\Big) = \frac{1}{n}\sum_{i=1}^n \Big( (X_i - \bar{X})Y_i\Big) 
}\end{equation} and very similar arguments (basically the same arguments
in reverse) also imply that
\begin{equation}\phantomsection\label{eq-hr2}{
  \frac{1}{n}\sum_{i=1}^n \Big( (X_i - \bar{X})X_i\Big) = \frac{1}{n}\sum_{i=1}^n \Big( (X_i - \bar{X})(X_i - \bar{X})\Big)
}\end{equation} We use both Equation~\ref{eq-hr1} and
Equation~\ref{eq-hr2} below.

\vspace{100pt}

Next, consider the numerator in Equation~\ref{eq-b1} \begin{align*}
...
covariance, the second equality holds by Equation~\ref{eq-hr1}, the
third equality plugs in for \(Y_i\), and the last equality combines
terms and passes the summation through the additions/subtractions.
...
\end{align*} where the first equality holds by Equation~\ref{eq-hr2} and
the second equality holds by the definition of sample variance.

For (C), well, we'll just carry that one around for now.

Plugging in the expressions for (A), (B), and (C) back into Equation
Equation~\ref{eq-b1} implies that \begin{align*}
...
\end{align*} where \begin{equation}\phantomsection\label{eq-V}{
\begin{aligned}
  V &= \E\left[ \left( \frac{(X - \E[X]) U}{\var(X)} \right)^2 \right] \nonumber \\
  &= \E\left[ \frac{(X - \E[X])^2 U^2}{\var(X)^2}\right] 
\end{aligned}
}\end{equation} which is what we were aiming for.
...
cell{2-13}{2}={}{halign=r},
cell{2-13}{3}={}{halign=r},
cell{2-13}{4}={}{halign=r},
cell{2-13}{5}={}{halign=r},
cell{2-13}{6}={}{halign=r},
cell{2-13}{7}={}{halign=r},
}                     %% tabularray inner close
...
\item
  ...
  \def\labelenumii{\alph{enumii})}
  \item
    Under homoskedasticity, the expression for \(V\) in
    Equation~\ref{eq-V} simplifies. Provide a new expression for \(V\)
    under homoskedasticity. \textbf{Hint:} you will need to use the law
...
1     0.898         0.898 1104188 1104301 1365.869
2     0.900         0.900 1103499 1103647 1370.001
3     0.928         0.928 1089372 1090328 2148.081
\end{verbatim}
...
\section{Introduction to Instrumental
Variables}\label{introduction-to-instrumental-variables}

As discussed in the previous chapter, for a variety of reasons, it is
often the case that it is infeasible to run an experiment to learn about
the causal effect of some treatment of interest. In this chapter, we
begin a discussion about common approaches to causal inference when an
experiment is not available. In this chapter, we will consider
approaches that are based on having access to a \textbf{natural
experiment}. These are not actual experiments in the sense that the
researcher randomly assigned the treatment, but instead arise when
``something weird'' happens that makes some individuals more likely to
participate in the treatment without otherwise affecting their outcomes.
This results in the treatment being effectively randomly assigned for
some subgroup. This ``something weird'' is called an
...
{Side Comment: } Instrumental variables has a strong claim for being the
main contribution of econometrics to statistics. It dates back to the
work of Philip Wright in the 1920s on supply and demand estimation in
agricultural markets -- it is more difficult than you might at first
think to estimate supply and demand curves. And I think that it is fair
to say that a lot of approaches to causal inference can be considered as
a variation of instrumental variables. Cunningham (2021) has a good and
interesting discussion of the history of instrumental variables in
economics.

\section{Setup}\label{setup}

We will continue to denote the outcome by \(Y\) and consider the case
with a binary treatment \(D\). We will also consider the case with a
binary instrument \(Z\). In the military service example, \(Y\) is a
person's earnings, \(D\) indicates whether they served in the military,
and \(Z\) indicates whether they were drafted. Our interest is in
learning about the causal effect of \(D\) on \(Y\), exploiting that
\(Z\) affects the probability of being treated and is effectively
randomly assigned.

\subsection{Potential Outcomes and Potential
Treatments}\label{potential-outcomes-and-potential-treatments}

We will continue to use the notation \(Y(1)\) and \(Y(0)\) to denote the
potential outcomes. Each person has potential treatment status depending
on the instrument: \(D_i(0)\) represents treatment status if not
drafted, and \(D_i(1)\) represents treatment status if drafted.
Similarly, each person has potential outcomes \(Y_i(0)\) and \(Y_i(1)\)
corresponding to earnings if they don't serve versus if they do serve.

Based on the pair \((D_i(0), D_i(1))\), every individual falls into one
of four types:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Compliers} have \(D_i(0) = 0\) and \(D_i(1) = 1\) - they serve
  only if drafted.
\item
  \textbf{Always-takers} have \(D_i(0) = 1\) and \(D_i(1) = 1\) - they
  serve regardless of draft status.
\item
  \textbf{Never-takers} have \(D_i(0) = 0\) and \(D_i(1) = 0\) - they
  never serve.
\item
  Finally, \textbf{defiers} would have \(D_i(0) = 1\) and \(D_i(1) = 0\)
  - they do the opposite of what the draft tells them.
\end{enumerate}

\subsection{The Four Assumptions}\label{the-four-assumptions}

We require four assumptions for the LATE theorem.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{relevance} requires that \(E[D_i(1)] > E[D_i(0)]\), meaning
  the instrument actually affects treatment on average.
\item
  \textbf{independence} requires that
  \(Z_i \perp (Y_i(0), Y_i(1), D_i(0), D_i(1))\), meaning the instrument
  is as-good-as randomly assigned.
\item
  the \textbf{exclusion restriction} requires that
  \(Y_i(D_i,z) = Y_i(D_i)\), meaning the instrument affects the outcome
  only through its effect on treatment.
\end{enumerate}

The most important threat to exclusion in the Vietnam draft lottery
context involves educational deferments. Men could avoid the draft by
enrolling in college and maintaining student status. This creates a
problem: even if a man with a low lottery number doesn't ultimately
serve (because he obtained a deferment), he might have enrolled in
college specifically to avoid military service.

This means the lottery can affect education directly, not just through
actual military service. Since education affects earnings, we have two
pathways:

\[Z_i \to D_i \to Y_i \quad \text{(allowed pathway through service)}\]
\[Z_i \to \text{Education}_i \to Y_i \quad \text{(violates exclusion - bypasses service)}\]

Consider a concrete example: John receives a low lottery number (high
draft risk) and enrolls in college to get a deferment. He never serves
in the military, so \(D_i = 0\). But his college enrollment was caused
by the lottery, and college affects his earnings. The lottery has
affected his outcome through a channel other than military service
itself.

This is a violation of the exclusion restriction because the instrument
(lottery number) has a direct effect on outcomes (through education)
that doesn't operate through the treatment (military service). When we
compute the IV estimate, we're attributing all of the earnings effects
to military service, but some of it is actually coming from education
choices induced by the lottery.

Angrist and Krueger investigated this concern empirically and found that
the lottery had relatively small effects on overall educational
attainment, suggesting this channel is not quantitatively large. But
this required careful investigation - the exclusion restriction is an
assumption about economics and behavior, not guaranteed by the random
assignment of lottery numbers. Even a perfectly randomized instrument
can violate exclusion if there are multiple causal pathways from the
instrument to the outcome.RetryClaude can make mistakes. Please
...
\tightlist
\item
  \textbf{monotonicity} requires that \(D_i(1) \geq D_i(0)\) for all
  \(i\), which rules out defiers.
\end{enumerate}

The monotonicity assumption is crucial and has a natural interpretation
in our draft lottery example: being drafted doesn't make you less likely
to serve. Defiers don't make sense in this context.

\subsection{The Proof}\label{the-proof}

The Wald estimator (equivalent to 2SLS in this simple binary case) is
defined as:

\[\text{IV estimate} = \frac{E[Y_i|Z_i=1] - E[Y_i|Z_i=0]}{E[D_i|Z_i=1] - E[D_i|Z_i=0]} = \frac{\text{Reduced Form}}{\text{First Stage}}\]

We now rewrite both the numerator and denominator using potential
outcomes notation. For the numerator, the independence assumption
implies that \(E[Y_i|Z_i=1] = E[Y_i(D_i(1))|Z_i=1] = E[Y_i(D_i(1))]\).

Similarly, \(E[Y_i|Z_i=0] = E[Y_i(D_i(0))]\). Therefore, the numerator
becomes \(E[Y_i(D_i(1))] - E[Y_i(D_i(0))]\). The denominator is simply
\(E[D_i(1)] - E[D_i(0)]\).

To understand what these expectations represent, we decompose them by
type.

For the numerator, consider each group's contribution. Never-takers have
\(Y_i(D_i(1)) = Y_i(0)\) and \(Y_i(D_i(0)) = Y_i(0)\), so they
contribute zero to the difference.

Always-takers have \(Y_i(D_i(1)) = Y_i(1)\) and
\(Y_i(D_i(0)) = Y_i(1)\), so they also contribute zero. Compliers have
\(Y_i(D_i(1)) = Y_i(1)\) and \(Y_i(D_i(0)) = Y_i(0)\), so they
contribute their individual treatment effect \(Y_i(1) - Y_i(0)\).
Defiers don't exist by monotonicity.

Let \(\pi_c\) denote the proportion of compliers in the population. Then
the numerator equals
\(\pi_c \cdot E[Y_i(1) - Y_i(0) | \text{complier}]\).

For the denominator, never-takers contribute
\(D_i(1) - D_i(0) = 0 - 0 = 0\), always-takers contribute
\(D_i(1) - D_i(0) = 1 - 1 = 0\), and compliers contribute
\(D_i(1) - D_i(0) = 1 - 0 = 1\). Therefore, the denominator equals
\(\pi_c\).
...
This is the Local Average Treatment Effect (LATE): the average treatment
effect for compliers.

\subsection{Interpretation and
Heterogeneity}\label{interpretation-and-heterogeneity}

The LATE theorem tells us that instrumental variables identifies the
effect of military service for those who serve because of the draft -
the compliers. Crucially, we do not learn about the treatment effect for
volunteers (always-takers), who may be positively selected, or for those
who would avoid service even if drafted (never-takers), who may have
high outside options.

This highlights the fundamental issue of treatment effect heterogeneity.
If the effect of military service differs across these groups - and
there are strong reasons to believe it would - then the IV estimate will
not equal the average treatment effect (ATE) in the population. The IV
estimate gives us a credible causal estimate, but only for a specific
subpopulation. This trade-off between internal validity and external
validity is central to interpreting instrumental variables estimates.

\section{Regression Discontinuity
Designs}\label{regression-discontinuity-designs}

Our next type of natural experiment is \textbf{regression discontinuity}
(often abbreviated RD or RDD for ``regression discontinuity design'\,').
In some sense, RD is a special case of instrumental variables; however,
discontinuities are common enough in economics that it is worth
considering these approaches specifically.

The basic idea of regression discontinuity is to exploit
``discontinuities'\,' in treatment assignment rules based on a running
variable. For examples, suppose that you are interested in how financial
aid (a treatment) affects college completion. Policies like the Hope
scholarship provide financial aid to students whose GPA is above a
certain cutoff (e.g., 3.0). Students with GPAs just above 3.0 receive
the treatment, while students with GPAs just below 3.0 do not. In the
terminology of regression discontinuity, the student's GPA is called a
\textbf{running variable}, which we will denote by \(R_i\). The cutoff
is denoted by \(c\) (in this case, \(c = 3.0\)). Treatment status
\(D_i\) is determined by whether the running variable exceeds the
cutoff: \(D_i = 1\) if \(R_i \geq c\) and \(D_i = 0\) if \(R_i < c\).

The key insight is that individuals just above and just below a cutoff
are likely to be similar on average, creating a local randomized
experiment at the threshold. This suggests that, by comparing outcomes
for students just above and just below the cutoff, we could hope to
estimate the causal effect of financial aid.

\subsection{Key Assumption: Continuity}\label{key-assumption-continuity}

The RD design relies on a \textbf{continuity assumption} about potential
outcomes. In particular, we assume that \begin{align*}
  \E[Y(1) | R = r] \text{ and } \E[Y(0) | R = r] \text{ are continuous in } r
\end{align*}

\subsection{Visualizing the Continuity
Assumption}\label{visualizing-the-continuity-assumption}

Below are two illustrative plots for the untreated potential outcome
conditional expectation function, (E{[}Y(0)\mid R=r{]}). The first plot
shows a case where the continuity assumption holds (the function varies
smoothly through the cutoff). The second plot shows a clear violation: a
jump at the cutoff even though treatment status changes there ---
indicating that the RD design would mistakenly attribute part of this
structural jump to the treatment effect.
...
{\centering \includegraphics{16-natural_experiments_files/figure-pdf/continuity_plot_continuous-1.pdf}

}

\caption{Illustration of continuity: E{[}Y(0)\textbar R=r{]} smooth at
cutoff}
...
{\centering \includegraphics{16-natural_experiments_files/figure-pdf/continuity_plot_violation-1.pdf}

}

\caption{Violation: jump in E{[}Y(0)\textbar R=r{]} at cutoff (vertical
segment removed)}

\end{figure}%

The jump in the second (red) curve at the cutoff would invalidate the
standard RD identification argument because (\lim\emph{\{r \uparrow c\}
E{[}Y(0)\textbar R=r{]} \neq \lim}\{r \downarrow c\}
E{[}Y(0)\textbar R=r{]}), so we could not substitute the limit from just
below the cutoff for the counterfactual just above.

Formally, we assume that the conditional expectation functions of both
potential outcomes are continuous at the cutoff:

\[\lim_{r \downarrow c} E[Y_i(0) | R_i = r] = \lim_{r \uparrow c} E[Y_i(0) | R_i = r] = E[Y_i(0) | R_i = c]\]

\[\lim_{r \downarrow c} E[Y_i(1) | R_i = r] = \lim_{r \uparrow c} E[Y_i(1) | R_i = r] = E[Y_i(1) | R_i = c]\]

In words: the expected potential outcome under control (no legal access)
varies smoothly through the cutoff, and the expected potential outcome
under treatment (legal access) also varies smoothly through the cutoff.
There are no jumps in the underlying potential outcomes at exactly age
21.

This is a strong but plausible assumption. It says that mortality risk
in the absence of legal drinking, \(E[Y_i(0)|R_i = r]\), doesn't
suddenly jump when someone turns 21. Similarly, mortality risk if
someone had legal access, \(E[Y_i(1)|R_i = r]\), doesn't jump for
reasons unrelated to the policy. The only thing that jumps at 21 is the
treatment status itself.

\subsection{Identification Under
Continuity}\label{identification-under-continuity}

To see how continuity delivers identification, observe that just below
the cutoff, everyone is untreated, so we observe:

\[\lim_{r \uparrow c} E[Y_i | R_i = r] = \lim_{r \uparrow c} E[Y_i(0) | R_i = r]\]

Just above the cutoff, everyone is treated, so we observe:

\[\lim_{r \downarrow c} E[Y_i | R_i = r] = \lim_{r \downarrow c} E[Y_i(1) | R_i = r]\]

The RD estimand is the difference:

\[\tau_{RD} = \lim_{r \downarrow c} E[Y_i | R_i = r] - \lim_{r \uparrow c} E[Y_i | R_i = r]\]

\[= \lim_{r \downarrow c} E[Y_i(1) | R_i = r] - \lim_{r \uparrow c} E[Y_i(0) | R_i = r]\]

Now apply the continuity assumption. Since \(E[Y_i(1)|R_i = r]\) is
continuous at \(c\):

\[\lim_{r \downarrow c} E[Y_i(1) | R_i = r] = E[Y_i(1) | R_i = c]\]

And since \(E[Y_i(0)|R_i = r]\) is continuous at \(c\):

\[\lim_{r \uparrow c} E[Y_i(0) | R_i = r] = E[Y_i(0) | R_i = c]\]

Therefore:

\[\tau_{RD} = E[Y_i(1) | R_i = c] - E[Y_i(0) | R_i = c] = E[Y_i(1) - Y_i(0) | R_i = c]\]

This is the average treatment effect for individuals at the cutoff. The
continuity assumption allows us to ``borrow'' the control group from
just below the cutoff to stand in for what would have happened to those
just above the cutoff in the absence of treatment.

\subsection{Threats to Continuity}\label{threats-to-continuity}

The continuity assumption can fail in several ways. First, if
individuals can precisely manipulate their value of the running
variable, then the populations just above and just below the cutoff may
differ systematically. For example, if students can retake a test to
barely pass a threshold, high-ability students might bunch just above
the cutoff while low-ability students remain below it. This would create
a discontinuity in underlying ability at the threshold, violating
continuity.

In our drinking age example, manipulation is implausible - individuals
cannot change their birth date to turn 21 sooner. However, other
policies might change discontinuously at age 21 (voting, certain
contracts, rental car eligibility), which could create discontinuities
in \(E[Y_i(0)|R_i = r]\) or \(E[Y_i(1)|R_i = r]\) for reasons unrelated
to alcohol access. We must argue that these other channels are either
absent or empirically small.

\subsection{The Connection to LATE}\label{the-connection-to-late}

The RD design identifies a \textbf{local} treatment effect in a similar
spirit to LATE, though the sense of ``local'' differs. While LATE is
local to compliers (a particular subpopulation defined by how they
respond to the instrument), the RD estimate is local to the cutoff value
(a particular point along the running variable).

This parallel becomes even clearer in fuzzy RD designs, where treatment
probability jumps at the cutoff but not from 0 to 1. For instance, not
everyone over 21 drinks alcohol, and some people under 21 obtain alcohol
illegally. In this case, let \(P_i\) denote actual alcohol consumption.
The fuzzy RD estimator is:

\[\tau_{FRD} = \frac{\lim_{r \downarrow c} E[Y_i | R_i = r] - \lim_{r \uparrow c} E[Y_i | R_i = r]}{\lim_{r \downarrow c} E[P_i | R_i = r] - \lim_{r \uparrow c} E[P_i | R_i = r]}\]

This is exactly a Wald estimator, where the cutoff serves as an
instrument for actual treatment! The fuzzy RD identifies a LATE: the
average treatment effect for compliers at the cutoff - those who drink
because they can legally do so at 21, but wouldn't drink if they were
just under 21.

The continuity assumption in fuzzy RD must apply to potential outcomes
as a function of actual treatment \(P_i\), not the instrument (being
over 21). We need \(E[Y_i(p)|R_i = r]\) to be continuous at \(c\) for
all treatment levels \(p\). This is analogous to the exclusion
restriction in IV: the running variable (age) affects outcomes only
through its effect on actual treatment (drinking).

\subsection{Treatment Effect Heterogeneity and External
Validity}\label{treatment-effect-heterogeneity-and-external-validity}

Just as LATE doesn't generally equal the average treatment effect when
effects vary across compliers and non-compliers, the RD estimate doesn't
generally equal the ATE when treatment effects vary along the running
variable. The effect of alcohol access on mortality at exactly age 21
may differ from the effect at age 25 or age 18.

Why might effects be heterogeneous along age? Younger drinkers may have
less experience with alcohol, be more prone to risk-taking behavior, or
face different social contexts. The mortality effect of legal access at
21 may be larger than it would be at 30. Formally, the treatment effect
function \(\tau(r) = E[Y_i(1) - Y_i(0)|R_i = r]\) need not be constant.
RD identifies \(\tau(c)\), while the ATE is \(E[\tau(R_i)]\).

This creates the same internal validity versus external validity
trade-off we saw with instrumental variables. The RD design provides a
very credible causal estimate by exploiting a sharp, visible
discontinuity that's difficult to manipulate. The as-if randomization at
the threshold gives us strong internal validity. However, we're only
learning about the treatment effect at one specific point, limiting
external validity.

\subsection{Practical Implications for
Interpretation}\label{practical-implications-for-interpretation}

When interpreting RD estimates of drunk driving laws, we should be clear
about what we're learning: the effect of legal alcohol access for
individuals right around their 21st birthday. If policymakers want to
know whether changing the drinking age to 18 or 25 would have similar
effects, the RD estimate may not directly answer that question. The
effect could be larger for younger, less experienced drinkers, or
smaller for more mature individuals.

Similarly, the complier interpretation in fuzzy RD matters for external
validity. We're identifying effects for marginal individuals - those
whose drinking behavior changes at the legal threshold - not for
everyone who drinks. Individuals who drink heavily regardless of
legality (always-takers) or abstain regardless (never-takers) don't
contribute to our estimate, just as in the IV case.

Both IV and RD designs share this fundamental feature: they trade broad
generalizability for credible causal identification at a specific
margin. Understanding treatment effect heterogeneity helps us recognize
both the power and the limitations of these natural experiment
approaches.
...
\includegraphics{16-natural_experiments_files/figure-pdf/unnamed-chunk-3-1.pdf}
...
\includegraphics{16-natural_experiments_files/figure-pdf/unnamed-chunk-5-1.pdf}
...
\chapter{Causal Inference with Observational
Data}\label{causal-inference-with-observational-data}
...
\bibitem[\citeproctext]{ref-cunningham-2021}
